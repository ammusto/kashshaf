...
/// Expand a query with proclitics for surface search with ignore clitics
fn expand_with_proclitics(query: &str) -> Vec<String> {
    let proclitics = ['و', 'ف', 'ب', 'ل', 'ك'];
    ...
    if words.len() == 1 {
        for p in &proclitics {
            variants.push(format!("{}{}", p, words[0]));
        }
    } else {
        let first = words[0];
        let rest = words[1..].join(" ");
        for p in &proclitics {
            variants.push(format!("{}{} {}", p, first, rest));
        }
    }
...
pub async fn concordance_search(
    ...
    // Build search terms based on mode and ignore_clitics
    let search_queries: Vec<String> = if mode == SearchMode::Surface && ignore_clitics {
        expand_with_proclitics(&query)
    } else {
        vec![query.clone()]
    };
...
pub fn export_concordance(
    ...
    let filename = format!("concordance_{}_{}.csv", sanitized_query, timestamp);
    let temp_dir = std::env::temp_dir();
    let file_path = temp_dir.join(&filename);

    let mut file = std::fs::File::create(&file_path)
        .map_err(|e| KashshafError::Database(format!("Failed to create CSV file: {}", e)))?;
    ...
    // Write data rows - export body text directly (no token fetching needed)
    for result in &results.results {
        writeln!(
            ...
            result.part_index,
            result.page_id,
            escape_csv(&result.part_label),
            escape_csv(&result.page_number),
            escape_csv(&result.body),

...
// Mirror of src-tauri/src/search.rs::AllDocsCollector. Caches FAST-field
// readers per segment in for_segment, reads triples directly in collect.
...
impl Collector for AllDocsCollector {
    ...
    fn merge_fruits(&self, segment_fruits: Vec<Self::Fruit>) -> tantivy::Result<Self::Fruit> {
        ...
        for v in segment_fruits { out.extend(v); }
        Ok(out)
    }
}
...
impl SearchEngine {
    ...
    /// Enumerate every matching page hit unscored. Returns (text_id, part_index,
    /// page_id) triples plus the Count-collector total. Mirror of the Tauri
    /// implementation; same algorithm, same query-build logic.
    pub fn collect_all_hits(
        ...
        // Collector returns triples directly via cached per-segment readers.

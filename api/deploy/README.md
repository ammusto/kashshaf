...
| `KASHSHAF_WARM_CACHE` | `1` | read the index, `boundary_index/` (4.3.0) and corpus.db once at startup; `/health.warm_cache` goes `pending` → `complete` (the deploy waits for it); unset = `disabled` |
